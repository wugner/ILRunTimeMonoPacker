    ...
    public class ILReflectJsonImport : AssetPostprocessor
    {
        ...
        public static void LoadJson()
        {
            ...
            if (File.Exists(fsmJsonPath) && string.IsNullOrEmpty(preference.FsmJsonPath) == false)
            {
                ...
                _fsmClassInfoList = JsonConvert.DeserializeObject<List<IrpClassInfo>>(stream);
                if (_fsmClassInfoList != null)
                {
                    _fsmClassTypes = _fsmClassInfoList.FindAll(linker => linker.IsSubClass == false).Select(linker => linker.ClassName).ToList();
                    _fsmClassTypes.Sort();
            ...
            if (File.Exists(monoJsonPath) && string.IsNullOrEmpty(preference.MonoJsonPath) == false)
            {
                ...
                var stream = sr.ReadToEnd();
                _monoClassInfoList = JsonConvert.DeserializeObject<List<IrpClassInfo>>(stream);
                if (_monoClassInfoList != null)
                {
                    _monoClassTypes = _monoClassInfoList.FindAll(linker => linker.IsSubClass == false).Select(linker => linker.ClassName).ToList();
